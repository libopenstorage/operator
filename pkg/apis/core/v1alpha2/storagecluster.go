...
// CloudDeviceSpec details the storage drive to be provisioned in cloud environment
type CloudDeviceSpec struct {
	// Type indicating the type of device as provided by a cloud provider
	// For ex. AWS (gp2,io1), GCE(pd-standard,pd-ssd)
	Type *string `json:"type"`
	// MinCapacity minimum capacity of for this cloud device spec
	MinCapacity resource.Quantity `json:"minCapacity"`
	// MaxCapcity capacity for this cloud device spec should not go above this threshold
	// Capacity of this CloudDevice can be either increased by expanding the volume
	// or adding device of the same type.
...
type CloudStorageSpec struct {
	...
	DeviceSpecs []*CloudDeviceSpec `json:"deviceSpecs,omitempty"`
	// JournalDeviceSpec spec for the journal device
	JournalDeviceSpec *CloudDeviceSpec `json:"journalDeviceSpec,omitempty"`
	// SystemMdDeviceSpec spec for the metadata device
	SystemMdDeviceSpec *CloudDeviceSpec `json:"systemMetadataDeviceSpec,omitempty"`
