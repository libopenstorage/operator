...
type StorageClusterLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.StorageCluster, err error)
...
// List lists all StorageClusters in the indexer.
func (s *storageClusterLister) List(selector labels.Selector) (ret []*v1alpha1.StorageCluster, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.StorageCluster))
	})
	return ret, err
}
...
type StorageClusterNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.StorageCluster, error)
...
// List lists all StorageClusters in the indexer for a given namespace.
func (s storageClusterNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.StorageCluster, err error) {
...
// Get retrieves the StorageCluster from the indexer for a given namespace and name.
func (s storageClusterNamespaceLister) Get(name string) (*v1alpha1.StorageCluster, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("storagecluster"), name)
	}
	return obj.(*v1alpha1.StorageCluster), nil
}
