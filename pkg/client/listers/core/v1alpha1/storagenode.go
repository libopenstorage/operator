...
type StorageNodeLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.StorageNode, err error)
...
// List lists all StorageNodes in the indexer.
func (s *storageNodeLister) List(selector labels.Selector) (ret []*v1alpha1.StorageNode, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.StorageNode))
	})
	return ret, err
}
...
type StorageNodeNamespaceLister interface {
	...
	Get(name string) (*v1alpha1.StorageNode, error)
...
// List lists all StorageNodes in the indexer for a given namespace.
func (s storageNodeNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.StorageNode, err error) {
...
// Get retrieves the StorageNode from the indexer for a given namespace and name.
func (s storageNodeNamespaceLister) Get(name string) (*v1alpha1.StorageNode, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("storagenode"), name)
	}
	return obj.(*v1alpha1.StorageNode), nil
}
