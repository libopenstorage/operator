...
type ClusterPluginInterface interface {
	...
	// GetStoragePodSpec given the storage cluster spec and the node name it returns the pod spec for that node
	GetStoragePodSpec(*corev1alpha2.StorageCluster, string) (v1.PodSpec, error)
