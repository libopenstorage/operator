...
func TestStorageClusterDefaultsForPlugin(t *testing.T) {
	...
	for i, td := range testData {
		...
		cluster.Status.DesiredImages.DynamicPluginProxy = "nginxinc/bar:1.23"
		err = driver.SetDefaultsOnStorageCluster(cluster)
		require.NoError(t, err)
		assert.Equal(t, "portworx/foo:1.0.0", cluster.Status.DesiredImages.DynamicPlugin,
			"failed expectations for test %d / %v", i+1, td)
		assert.Equal(t, "nginxinc/bar:1.23", cluster.Status.DesiredImages.DynamicPluginProxy,
			"failed expectations for test %d / %v", i+1, td)
		...
		if td.expectUpdated {
			assert.Equal(t, "portworx/portworx-dynamic-plugin:1.1.0",
				...
				"failed expectations for test %d / %v", i+1, td)
			assert.Equal(t, "nginxinc/nginx-unprivileged:1.25",
