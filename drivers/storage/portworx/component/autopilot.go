...
func (c *autopilot) createDeployment(
	...
	if pxutil.IsTLSEnabledOnCluster(&cluster.Spec) {
		...
		//    - name: PX_ENABLE_TLS
		//	    value: "true"
		logrus.Infof("Secret name containing CA cert: %v", pxutil.DefaultCASecretName)
		intermediateEnvVars = append(intermediateEnvVars,
			v1.EnvVar{
				Name:  pxutil.EnvKeyAutopilotCASecretName,
				Value: pxutil.DefaultCASecretName,
			},
		)
		logrus.Infof("Secret key containing CA cert: %v", pxutil.DefaultCASecretKey)
		intermediateEnvVars = append(intermediateEnvVars,
			v1.EnvVar{
	...
	for _, env := range intermediateEnvVars {
